...
\todo[inline]{Make sure there's a good loop diagram in that section pointed below, and that the enumerated parts are explained in detail}
From the follow loop presented in section \ref{sec:follow} it is possible to divide the processing cost in several sections that can be measured independently: image processing, offboard control, keyboard input and released thread.
...
Figure \ref{fig:perf-sitl-sim} shows the time used for each of the tasks on an average run of the follow solution with simulated hardware.
...
This is where the biggest differences in performance will come from between the simulated hardware and the solution running in the Pixhawk 4 + Raspberry Pi combination.
...
The three subtasks that make up the image processing are:
\begin{enumerate}
    \item Get frame: request new frame from the video source.
    \item Process: send frame to MediaPipe library for detection and/or tracking.
    \item Detect: calculate bounding box coordinates and define valid pose.
\end{enumerate}
...
Figure \ref{fig:perf-analysis} shows the measurements taken for all the hardware combinations analysed.

\todo[inline]{Performance: Take measurements and comment results, calculate max speed for each, say its enough}
