...
With the results matching exactly those obtained in section \ref{sec:test-3-airsim}, and since the flight stack is now running on a physical controller, it is possible to add an RC antenna to the \verb|PPM RC| \todo{check port} port of the board to be able to fly the vehicle with a remote control unit after it has been bound to the receiver \footnote{\url{https://docs.px4.io/main/en/config/radio.html}}.
...
As can be seen in figure \ref{fig:rc-tests}, \todo{...} ....
...
\todo[inline]{Measure voltage and amperage through power ports separately and together}
\todo[inline]{Close up image and description of custom connector}
...
This can be disabled through the \verb|raspi-config| command-line utility (Advanced config -> Serial -> Disable\todo{Check option path on latest OS}).
...
\todo[inline]{Performance analysis with/out peripherals, detection only, detection and control together}
