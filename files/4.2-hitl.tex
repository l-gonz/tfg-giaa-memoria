...
From the follow loop presented in section \ref{sec:follow} it is possible to divide the processing costs in several sections that can be measured independently:
\begin{enumerate}
    \item Image processing
    \item Offboard control
    \item Keyboard input
    \item Released thread
\end{enumerate}

Figure \ref{} shows the time used for each of the tasks on an average run of the follow solution with simulated hardware. It is therefore the first item the one to be highlighted for further analysis.
