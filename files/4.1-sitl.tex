...
For this purpose and to be able to run with a minimal configuration, the Gazebo simulator \todo{cite} included with the base PX4 installation will be used as the 3D environment.
...
\todo[inline]{Get a better image for this}
\begin{figure}
  \centering
...
\todo[inline]{Save video on GitHub and put link here, fix text to make sense}
...
\todo[inline]{Image of PX4 console + Unreal with Airsim scene}
\begin{figure}
  \centering
...
\todo[inline]{Image of dronecontrol console + Unreal with Airsim scene + image output running test-camera}
\begin{figure}
  \centering
  \includegraphics[width=0.6\textwidth, keepaspectratio]{img/placeholder.png}
...
The drone movement when running the follow control program with values of 10 and 2 on the proportional term of the yaw and forward controllers, respectively, which can be done with the command \mintinline{bash}{dronecontrol follow --sim --yaw-pid (10, 0, 0) --fwd-pid (2, 0, 0)}, is shown in video format in \todo{video} this link and a frame extracted from it can be seen in figure \ref{fig:airsim-test-follow}.
