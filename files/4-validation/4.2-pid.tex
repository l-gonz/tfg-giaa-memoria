...
During calibration, the vehicle and the target person always start in the same reference position. In the world coordinates of the simulated environment, the positions are x=0 and y=0 for the vehicle and x=600 and y=0 for the person, as depicted in Figure \ref{fig:tune-start-pos}.
At these positions, the person is detected centred in the field of view, and its calculated height takes up 36\% of the image height. Consequently, the controllers running within the simulator will have target set points of 0.5 for the yaw controller and 0.36 for the forward controller.
The result of these target points is that for any changes in the position of the simulated person, the controllers will transmit velocity commands to the autopilot to achieve the same relative position between the vehicle and the person as between the reference positions.
...
\todo[inline]{Join with final video}
...
A video showcasing the drone's movement with these parameter values can be viewed \href{https://l-gonz.github.io/tfg-giaa-dronecontrol/videos/test-sitl-follow}{here}\footnote{\url{https://l-gonz.github.io/tfg-giaa-dronecontrol/videos/test-sitl-follow}}, and a frame extracted from the video is displayed in Figure \ref{fig:airsim-test-follow}.
