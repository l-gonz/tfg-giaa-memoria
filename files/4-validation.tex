...
\todo[inline]{Image: Videos for github}
...
\todo[inline]{Write: Discuss velocity graphs as well for every section, mention velocity limits}
...
\todo[inline]{Test: Controller should end up with (4, 0.1, 0)}
...
It also creates an effect, especially for the bigger $K_P$ values tested, where even as the vehicle begin its movement backwards to its target position, the pose detection mechanism get a better image of the followed person, increasing or decreasing the detected height slightly, so that small spikes of height differences show in the height graphs even though the velocity graph shows that the vehicle's direction of movement remains the same during those approximately 3 seconds at the beginning of each test.
This is the main reason why the output forward velocity needs to be kept small in the controller.
In the right graph of figure \ref{fig:tune-fwd-prop}, it is also visible for high values of $K_P$ how the output velocity is high enough that the maximum velocity limit is reached for the forward controller, which is set at 0.4\unitfrac{m}{s}.
...
Furthermore, the whole testing process followed with the developed \texttt{test-controller} tool can be seen in \todo{Image: link video of test-controller tool}.
...
\todo[inline]{Write: TODO validate fwd, and rerun}
...
A video of the full follow solution running with the appropriate tunings for the controllers can be seen in \todo{Image: Link video to follow run}.
