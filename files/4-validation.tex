...
\todo[inline]{Discuss velocity graphs as well for every section, mention velocity limits}
...
\todo[inline]{Controller should end up with (4, 0.1, 0)}
...
As a final validation for the tuning obtained previously, in this section the \verb|test-controller| tool described in section \ref{subsec:test-tools} will be used check the step response of the controllers to different starting distances for the same coefficients.
...
Furthermore, the whole testing process followed with the developed \verb|test-controller| tool can be seen in \todo{link video of test-controller tool}.
...
Additionally, the \verb|test-controller| tool is run with varying positions in the x axis, which means that the tests are run with the human figure at different distances closer and further away from the vehicle, while maintaining in the center of its field of view (y position will remain 0 for the whole process).
...
A video of the full follow solution running with the appropriate tunings for the controllers can be seen in \todo{Link video to follow run}.
